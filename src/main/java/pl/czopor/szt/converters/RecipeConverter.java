...
public class RecipeConverter implements Converter<Recipe, RecipeDto> {
	...
	public Recipe mapFromDto(RecipeDto recipeDto) {
		...
		User user = userConverter.mapFromDto(recipeDto.user);
		Recipe recipe = Recipe.builder().id(recipeDto.id).user(user).description(recipeDto.description).images(images)
				.duration(recipeDto.duration).mark(recipeDto.mark)
	...
	public RecipeDto simpleMapToDto(Recipe recipe) {
		...
		UserDto user = userConverter.mapToDto(recipe.getUser());
		return RecipeDto.builder().id(recipe.getId()).user(user).description(recipe.getDescription()).images(images)
				.duration(recipe.getDuration()).mark(recipe.getMark()).recipeType(recipeType)
				...
				.userActivity(activityService.getUserRecipeActivity(recipe)).build();
	}
